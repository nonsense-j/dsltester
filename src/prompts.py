...
as "// (Should Alert / Shouldn't Alert): {{checking_scenario}}". Never mention test index in the test code since the order may be changed in the future.
...
During summarization, you must also carefully analyze every existed regex pattern and its matching logic.
...
7. **Standardized Test Code**. Each test must include a main public class, whose name indicates the checking scenario it covers concisely. \
Also, add clear and targeted comments for each test, where the first line of the test should be a general comment describing the checking scenario \
as "// Should Alert: {{checking_scenario}}". Never mention test index in the test code since the order may be changed in the future.
...
7. **Standardized Test Code**. Each test must include a main public class, whose name indicates the checking scenario it covers concisely. \
Also, add clear and targeted comments for each test, where the first line of the test should be a general comment describing the checking scenario \
as "// Shouldn't Alert: {{checking_scenario}}". Never mention test index in the test code since the order may be changed in the future.
...
# General Goal
Given a DSL-formatted code checker and Java-coded test cases that don't trigger alerts, modify each test case so the checker reports it. \
Ensure that the output test code aligns with the checker's logic.

# Critical Rules for Refinement
1. **Trigger Checker Reporting**
    - Compare the checker's violation patterns with the test code to find why the original test wasn't reported.
    - Make targeted changes only where needed to match the checker's detection logic.
2. **Preserve Core Identity**
    - Keep original main class name, existing imports and core logic. 
    - All modifications must yield compilable Java files
    - Retain or refine comments to highlight the checking scenario for each test and never mention the test index in any comment.
...
### Test Cases Should be Reported by Checker
{wrapped_tests}

### Output
Step 1: For each test case, first compare the checker's violation patterns with the test code to determine whether the test already meets the checking logic.
Step 2: For each test case: If modifications are needed, directly output the refined test code wrapped with "<alerting_test>" and </alerting_test>. \
If the original test already meets the checking logic, directly output "<alerting_test> //skip </alerting_test>" instead of repeating the original code.
"""
